...
D先生 : “さて、いきなりDQN with Experience Replayをやることになるらしいですが・・・。QEUも昔よりも少しだけレベルが上がったとはいえ、このテーマでDQN-ERを使うとは「日本刀で大根を切る」感覚は否めません。”
...
class Agent():
    ...
    def get_episode(self, iCnt_play):
        ...
        while True:
            ...
            # ----------------
            # 実行継続するか分岐
            if done == True or iCnt_turn > 20000 or reward < -20:
                # Game over
                break # (game over rip)
            else:
                # count of game turn
...
QEU:FOUNDER ： “学習の結果はこんな感じです。思ったよりも学習のパフォーマンスが悪かったなァ・・・。なにか、（プログラムが）間違っているかもしれんが・・・。”
...
QEU:FOUNDER（60代の設定です） ： “まさか、これを**「お蔵出し」する**とは思わなかった。”
