## QEUR21_RLCLF2 – 「崖歩き(Cliff_Walking)」をさらっと復習しましょう(その3)

## ～　さらにレベルアップをめざす　～

### ・・・　まあ、「（前回の）つづき」ということで　・・・

D先生 : “前回で**DQN with Experience Replayのベースライン**ができたので、これにメトリックスを加味するんでしょ？・・・で・・・、どのようなメトリックスですか？”

![imageRL3-3-1](https://QEUWIndValley.github.io/images/imageRL3-3-1.jpg)

QEU:FOUNDER ： “スタート距離とゴール距離を両方とって学習に入れています。メトリックスとしては、ゴール距離とスタート距離を合計して「トータル距離」を計測しています。このようにすれば、崖(Cliff)からの離れ具合を計測できます。”

D先生 ： “さて、その効果はどうかな・・・。その答えはプログラムをドン！！。”


```python
# ----------------
# CLIFF WALKING SIMULATIONの強化学習システム
# step2 : 「ちくわ(距離)」を加えたうどん（強化学習モデル）です
# step2 : dqn_wER_cwSimu(agent_distance).py
# ---------------- 
# 数値計算のﾗｲﾌﾞﾗﾘ読み込み
...
class Agent():
    ...
    def get_episode(self, iCnt_play):
        ...
        state, state_next = np.zeros(50), np.zeros(50)
        ...
        # ---------------------------
        # 状態(STATE_NEXT)を生成する
        state = np.hstack([temp_state, [EuDist_S, EuDist_G]])
        ...
        while True:
            ...
            # ---------------------------
            # 状態(STATE_NEXT)を生成する
            state_next = np.hstack([temp_state_next, [EuDist_S_next, EuDist_G_next]])
...
QEU:FOUNDER ： “少しだけ良くなったかな・・・。”

**（今回）**

![imageRL3-3-2](https://QEUWIndValley.github.io/images/imageRL3-3-2.jpg)

**（前回、ベースライン）**

![imageRL3-3-3](https://QEUWIndValley.github.io/images/imageRL3-3-3.jpg)

D先生 : “Q値の動きを見ると効果を感じますね。”

**（今回）**

![imageRL3-3-4](https://QEUWIndValley.github.io/images/imageRL3-3-4.jpg)

**（前回、ベースライン）**

![imageRL3-3-5](https://QEUWIndValley.github.io/images/imageRL3-3-5.jpg)

QEU:FOUNDER ： “距離の導入により、やはり確実にレベルアップしています。**Q値が少し高くなる**んだよね、学習の後ろの方で・・・。”

![imageRL3-3-6](https://QEUWIndValley.github.io/images/imageRL3-3-6.jpg)

D先生 : “このやり方は、以前メイズ（迷路）でやった方法ですね。やっぱり、**ゴール距離を入力情報に持っていると学習の収束が良くなります**。”

QEU:FOUNDER ： “じゃあ、次もメイズと同様に、もう一つのメトリックスを作りましょう。”

## ～　まとめ　～

D先生 : “おや？「めずらしい人？」の紹介ですね。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/ST1G8Tw8Uek" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

QEU:FOUNDER ： “おもしろいでしょ・・・？**天才Youtuber**らしいです。”

![imageRL3-3-7](https://QEUWIndValley.github.io/images/imageRL3-3-7.jpg)

D先生 : “政治のネタが中心で3万人の登録者だから、すごいですよね。”

QEU:FOUNDER ： “おまけに、ホームグラウンドが地方ですからね。

D先生 : “やっぱり・・・、「天才」じゃないの？”

![imageRL3-3-8](https://QEUWIndValley.github.io/images/imageRL3-3-8.jpg)

QEU:FOUNDER ： “まあ・・・、イケメンよりも天才度は少し落ちるが・・・。“
