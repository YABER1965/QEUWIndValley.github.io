## QEUR21_RLCLF5 – 「崖歩き(Cliff_Walking)」をさらっと復習しましょう(その6)

## ～　ONEHOT法の適用で学習結果はどうなる？　～

D先生 : “さて・・・、次はどうしよう・・・。やっぱり、なるべく関数を簡単にしたいし・・・。”

QEU:FOUNDER ： “じゃあ、**ONE-HOT(encoding)にすればいい**んじゃないですか？”
...
# step5 : 「リベンジ豊橋産ちくわ」を加えたうどん（強化学習モデル）です
# step5 : dqn_wER_cwSimu(agent_simple).py
# ---------------- 
# 数値計算のﾗｲﾌﾞﾗﾘ読み込み
...
def calc_address(number_state):
    ...
    i = int(number_state/COLS+0.1)
    j = int(number_state - i*COLS+0.1)
...
def calc_availaction(number_state):
    ...
    # 二次元アドレス化
    row, col = calc_address(number_state)
    ...
    def choose_action(self, state, number_state, iCnt_turn):
        ...
        if self.epsilon <= random.random():
            ...
            for action in arr_avail:
                ...
                #print("a:",a)
                # actionをONEHOTに変換する
                a_onehot = calc_onehot(action)
                temp_s_a = np.hstack([state, [a_onehot]])
                if iCnt == 0:
        ...
        for ibat in range(batch_size):
            ...
            if done:
                ...
                for imov in arr_avail:
                    # actionをONEHOTに変換する
                    a_onehot = calc_onehot(imov)
                    # [array]-action を結合します。
                    temp_s_a = np.hstack([state_next, [a_onehot]])
                    if iCnt == 0:
...
class Agent():
    ...
    def get_episode(self, iCnt_play):
        ...
        state, state_next = np.zeros(state_size), np.zeros(state_size)
        ...
        temp_state, a_row, a_col = calc_address(number_state)
        ...
        while True:
            ...
            # 学習の進捗を表示する（一次元アドレスと二次元アドレスに変換する）
            temp_state_next, a_row_next, a_col_next = calc_address(number_state_next)
            #print("iCnt_play:{}, iCnt_turn:{}, a_row:{}, a_col:{}, a_order:{}, reward:{}".format(iCnt_play, iCnt_turn, a_row, a_col, a_order, reward))
...
QEU:FOUNDER ： “それでは、学習結果をドン！！おっと・・・、これは惜しい・・・。”

**（学習曲線）**

![imageRL3-6-1](https://QEUWIndValley.github.io/images/imageRL3-6-1.jpg)

**（パフォーマンス推移）**

![imageRL3-6-2](https://QEUWIndValley.github.io/images/imageRL3-6-2.jpg)

D先生 : “もう一歩、届かなかったか・・・。でも、**これで問題点がはっきりしました**ね。おっと、念のため・・・。DL関数のストラクチャを変えてみてどうでしたか？”

QEU:FOUNDER ： “関数のレイヤをちょっと深くしてみましたが、基本は同じ傾向です。パフォーマンスは若干良くなくなりましたが、48次元のときのようなパフォーマンスにはならないでしょう。”

D先生 : “今回はケーススタディとして、わざとゆっくり進行しましたが、面白かったですね。。”

QEU:FOUNDER ： “メトリックスの効用、及びONEHOT化の特性が見えてきたように思います。次は何をすればいいでしょうか？**POLICY-BASED法であるREINFORCE**をやってみますか？”
...
QEU:FOUNDER ： “しばらくイケメン関連の話をやり過ぎたので、今回は一回休み・・・。この話をしましょう。クルマ・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/akXi3p9rC0A" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

D先生 : “FOUNDERの好きなメディアですね。まあ、J国のメディアは束になっても勝てない・・・。ちなみに、この動画は中文字幕つきですよ。”

![imageRL3-6-3](https://QEUWIndValley.github.io/images/imageRL3-6-3.jpg)

QEU:FOUNDER ： “最近、J国の〇田首相がT社という大きな車メーカに訪問した件について報じています。国際的に、「電気自動車とハイブリッド自動車は同等に環境にやさしいクルマであると世界に発信してほしい」と要望したそうです。”

D先生 : “及び、国内でエコポイントなどの優遇制度があれば電動車とハイブリッド車は同様の取り扱いにしてほしいと・・・。”

QEU:FOUNDER ： “この動きが最近のEUが2035年までに電動車のみを購入するという5年遅れの政策変更につながったと・・・。この業界ではJ国もこれほどの力があると、ちょっと見直したわ・・・。”

D先生 : “これが政権与党の強さだネ・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/4MjQtkoJISc" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

QEU:FOUNDER ： “どうりで、なかなか選挙で負けないわけだ（笑）。”
